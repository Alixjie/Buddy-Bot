...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->start_lidar, &QPushButton::clicked, this,
          &MainWindow::start_lidar);
  connect(ui->end_lidar, &QPushButton::clicked, this, &MainWindow::end_lidar);

  connect(ui->start_obstacle_avoidance, &QPushButton::clicked, this,
          &MainWindow::start_obstacle_avoidance);
  connect(ui->end_obstacle_avoidance, &QPushButton::clicked, this,
          &MainWindow::end_obstacle_avoidance);

  connect(ui->quit_program, &QPushButton::clicked, this, &QApplication::quit);

  connect(ui->voice_control_start, &QPushButton::clicked, this,
          &MainWindow::voice_control_start);
  connect(ui->voice_control_stop, &QPushButton::clicked, this,
          &MainWindow::voice_control_stop);
  connect(ui->start_follow, &QPushButton::clicked, this,
          &MainWindow::car_control_main);
  connect(ui->stop_follow, &QPushButton::clicked, this,
  ...
  // Periodically calling timercallback to change the motion status and keep
  // speed-loop.
  ...
  if (SL_IS_FAIL(lidar->connect(channel)))
    std::cerr << "Error, cannot bind to the specified serial port /dev/ttyUSB0"
              << std::endl;
  ...
  if (SL_IS_FAIL(operat_result)) {
    if (operat_result == SL_RESULT_OPERATION_TIMEOUT)
      ...
      std::cerr << "Error, unexpected error, code: " << operat_result
                << std::endl;
  }
  ...
  if (SL_IS_OK(operat_result)) {
    ...
    std::cerr << std::endl
              << "errorcode: " << health_info.error_code << std::endl;

  } else
    std::cerr << "Error, cannot retrieve the lidar health code: "
              << operat_result << std::endl;

  if (health_info.status == SL_LIDAR_STATUS_ERROR) {
    std::cerr << "Error, slamtec lidar internal error detected. Please reboot "
                 "the device to retry"
              << std::endl;
    lidar->reset();
  }
  ...
  if (num_paint > 10) {
    ...
    if (system_status == lidar_scan) {
      ...
      for (int num = 0; num < (int)count; ++num) {
        //        std::cout << "Theta: " << std::fixed << std::setprecision(2)
        //        << nodes[num].angle_z_q14 * 90.f / 16384.f; std::cout << "
        //        Distance: " << nodes[num].dist_mm_q2 / 4.0f; std::cout << "
        //        Quality: " << (nodes[num].quality >>
        //        SL_LIDAR_RESP_MEASUREMENT_QUALITY_SHIFT) << std::endl;

        if ((nodes[num].quality >> SL_LIDAR_RESP_MEASUREMENT_QUALITY_SHIFT) !=
            0) {
          double x =
              origin_point.x() +
    ...
    if (system_status == obstacle_avoidance) {
      ...
      for (int num = 0; num < (int)count; ++num) {
        ...
        if (((nodes[num].quality >> SL_LIDAR_RESP_MEASUREMENT_QUALITY_SHIFT) !=
          ...
          //                    std::cout << "Point x y : " << x << ", " << y <<
          //                    " " << (nodes[num].angle_z_q14 * 90.f / 16384.f)
          //                    << " ";
          painter.drawPoint(QPoint(int(x), int(y)));
        }
      }
      ...
      for (std::list<Point>::iterator it = returnList.begin();
           it != returnList.end(); ++it) {
        painter.drawPoint(QPoint(int(it->x) * 10, int(it->y) * 10));
        //                if ((it->x == star.getEndPoint().x) && (it->y ==
        //                star.getEndPoint().y))
        //                    std::cout << "( " << it->x << ", " << it->y << "
        //                    )";
        //                else
...
void MainWindow::timerEvent(QTimerEvent *event) {
  if (system_status == lidar_scan) {
    ...
    star.setEndPoint(Point(int(uwb_label_position.x() / 10),
                           int(uwb_label_position.y() / 10)));
...
void MainWindow::end_lidar() { killTimer(timer_id_lidar_scan); }
...
void MainWindow::voice_control_start() { vc.voicecontrol_start(); }
