...
char* string_duplicate(const char* source) {
  ...
  char* result = (char*)malloc(length + 1);  // change void* to char*
  strncpy(result, source, length + 1);
  return result;
}

void string_list_add(const char* new_string, char*** list, int* list_length) {
  *list = (char**)realloc(*list, (*list_length + 1) * sizeof(char*));  // change void* to char**
  (*list)[*list_length] = string_duplicate(new_string);  
  *list_length += 1;
}
...
void get_input_devices(char*** devices, int* devices_length) {
  ...
  *devices_length = 0;
  ...
  for (int ctr = 0; ctr < 16; ctr++) {
    ...
    const char* device_name = pa_input_devicelist[ctr].name;
    string_list_add(device_name, devices, devices_length);
  }
}